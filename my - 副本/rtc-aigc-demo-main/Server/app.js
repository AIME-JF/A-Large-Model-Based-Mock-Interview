...
app.use(async ctx => {
  ...
  await wrapper({
    ...
    logic: async () => {
      ...
      console.log(`[VOLC-REQ] Action=${Action} body=`, JSON.stringify(body, null, 2));
      ...
      console.log(`[VOLC-RESP] Action=${Action} status=${result.status} resp=`, JSON.stringify(json, null, 2));
      return {
        ...json,
        ProxyMetadata: {
          action: Action,
          ...
          ok: result.ok,
          requestId: json?.ResponseMetadata?.RequestId || json?.ResponseMetadata?.RequestID || '',
        },
      };
    }
  });
