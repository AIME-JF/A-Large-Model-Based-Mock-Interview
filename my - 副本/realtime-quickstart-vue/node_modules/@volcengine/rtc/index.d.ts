...
declare class VERTC implements IVERTC {
	...
	setLogConfig: ({ logLevel, LogfileSize, }: {
		logLevel?: "none" | "warning" | "error" | "info" | "trace" | undefined;
...
declare const Config: {
	...
	ICE_CONFIG_REQUEST_URLS: string[];
	...
	UPLOAD_REMOTE_STATS: ExtendMediaType;
...
declare function getParameter(key: keyof typeof Config): {};
...
export declare enum SubscribeFallbackOption {
	...
	DISABLE = 0,
	/** 
	 * @brief 下行网络不佳时，对视频流做降级处理。具体降级规则参看[音视频流回退](https://www.volcengine.com/docs/6348/70137)文档。
	 *        该设置仅对发布端调用 [EnableSimulcastMode](104478#enablesimulcastmode) 开启发送多路流功能的情况生效。
	 */
...
export interface IEngineEvents {
	...
	/** 
	 * @brief 调用 [enableAudioPropertiesReport](104478#enableaudiopropertiesreport) 后，根据设置的 interval 值，你会周期性地收到此回调，了解本地音频的相关信息。
...
export interface IRTCEngine extends EnhancedEventEmitter<IEngineEvents> {
	 ...
	 * @param roomId 加入的房间 ID。该字符串需符合正则表达式：`[a-zA-Z0-9_@\-\.]{1,128}`。
	 * @param userInfo 用户信息。
	...
	/** 
	 * @brief 手动开启音视频流播放。
	 * @param userId 指定流所属的用户 ID 或 公共流 ID。空值代表本地媒体流。传入公共流 ID 时，`mediaType` 和 `streamIndex` 不生效。
	 ...
	 * @param message SEI 消息，不大于 4 KB。当消息为空或大小超限，终止发送，并收到相应的错误提示。
	 * @param repeatCount 消息发送重复次数，SEI 数据会添加到当前视频帧开始的连续 `repeatCount+1` 个视频帧中。取值范围 `[0, min{30, %{视频帧率}×2}]`，推荐范围 `[2,4]`。
	 * @notes + 功能兼容性说明：<br/>
	 *         &ensp; - SEI 消息仅支持在 H.264 编码，且使用 Chromium 86+ 内核的浏览器上使用。
	...
	/** 
	 * @brief 在视频发布端设置发布的视频流参数。
	 * @param descriptions 要推送的多路视频流的参数，所设置的分辨率是各路流的最大分辨率。
	 ...
	 * @notes + 调用该方法前，SDK 默认发布一条分辨率 640 × 480 px，帧率 15 FPS，最大编码码率 600 Kbps 的流。
	...
	/** 
	 * @brief 发布一路公共流。
	 * @param publicStreamId 公共流 ID。该字符串需符合正则表达式：`[a-zA-Z0-9_@\-\.]{1,128}`。
	 * @param publicStreamParam 公共流参数。
	 *                 一路公共流可以包含多路房间内的媒体流，按照指定的布局方式进行聚合。
	 *                 如果指定的媒体流还未发布，则公共流将在指定流开始发布后实时更新。
	 * @return `REPEAT_PUSH`: 重复发布公共流。
	 * @notes + 使用同一 `appID` 的用户，无论是否进房，可以调用 [startPlayPublicStream](#startplaypublicstream) 获取和播放指定的公共流。
	...
	/** 
	 * @brief 获取已订阅的公共流的音频或者视频的 [MediaStreamTrack](https://developer.mozilla.org/en-US/docs/Web/API/MediaStreamTrack)。
	 * @param id 公共流 ID。
	 * @param type 媒体流类型，视频或音频。
	 * @return 当 MediaStreamTrack 不存在时，比如没有订阅该公共流，返回 `undefined`。
	 * @notes + 调用时机，调用 [`startPlayPublicStream`](#rtcengine-startplaypublicstream) 订阅公共流后。
	 ...
	 * @notes + 本地视频处理的效果，镜像、美颜和虚拟背景均包含在截取的画面中。
	...
	/** 
	 * @brief 调节本地播放的公共流播放音量。
	 * @param publicStreamId 音频所属公共流 ID
	 * @param volume 播放的音量值和原始音量的比值，取值范围 [0, 400]，单位 %。默认值为 100%。
	 *               为保证更好的通话质量，建议将 volume 值设为 [0,100]。
	 * @notes + 建议成功播放公共流后设置音量。
...
export interface PublicStreamConfig {
	...
	video?: {
		...
		/** 
		 * @brief 高度，单位：px。
		 *        仅支持设为偶数，如果设为奇数，云端媒体处理会直接拒绝，导致公共流失败。
		 */
